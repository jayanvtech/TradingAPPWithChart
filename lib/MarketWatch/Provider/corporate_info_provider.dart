...
//CorporateInfo? get corporateInfo => _corporateInfo;  // Getter returns nullable CorporateInfo
  ...
  Future<void> fetchCorporateInfo(String symbol) async {
    final url = 'http://180.211.116.158:8080/mobile/api/v1/stock-analysis/top-corp-info-equity?symbol=$symbol';
    try {
      _isLoading = true;
      notifyListeners();
...
class CorportateProvider with ChangeNotifier {
  StockCorporateDetailModel? irctcData;

  Future<void> fetchIRCTCData() async {
    final url = 'http://180.211.116.158:8080/mobile/api/v1/stock-analysis/top-corp-info-equity?symbol=IRCTC'; // Replace with your actual API endpoint
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        irctcData = StockCorporateDetailModel.fromJson(data['data']);
        notifyListeners();
      } else {
