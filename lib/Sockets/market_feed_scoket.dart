...
class MarketFeedSocket extends ChangeNotifier {
  ...
  Future<void> connect() async {
    ...
    String url = '$baseUrl?token=$token&userID=A0031&publishFormat=JSON&broadcastMode=Full&apiType=APIMARKETDATA';
    ...
    socket.on('order', (error) {
      print('ORdsfser: $error');
    });
    // socket.on('1505-json-full', (Data) {
    //   print('Real Time Candle data: $Data');
    // });
    // socket.on('1502-json-full', (Data) {ððð
    //   print('1540222222222222: $Data');
    // });
    //   socket.on('1512-json-full', (Data) {
    //   print('Real Time Candle data: $Data');
    // });
    //   socket.on('1502-json-full', (error) {
    //   print('ORdsfser: $error');
    // });
    socket.on('position', (error) {
      print('dpostioon value: $error');
    });
